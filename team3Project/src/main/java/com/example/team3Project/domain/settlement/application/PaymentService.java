...
public class PaymentService {
    ...
    public Payment processPayment(PaymentRequest request) {
        ...
        if (!card.isActive()) {
            return failPayment(order, payment, "CARD_INACTIVE");
        }

        if (order.getTotalAmount() > card.getCardLimit()) {
            return failPayment(order, payment, "FAILED_LIMIT");
        }

        if (order.getTotalAmount() > card.getBalance()) {
            return failPayment(order, payment, "FAILED_BALANCE");
        }
    ...
    // 결제 실패
    private Payment failPayment(Order order, Payment payment, String status) {
